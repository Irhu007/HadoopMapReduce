...
public class SalaryHike {
	...
	Configuration conf = new Configuration();
        DistributedCache.addCacheFile(new URI("hdfs://localhost:54310/user/hduser/des.txt"), conf);
	...
	Job job_obj = new Job(conf, "Salary Hike"   );
	// setting name of main class
       // job_obj.addCacheFile(new Path("hdfs://localhost:54310/user/hduser/des.txt").toUri());
	job_obj.setJarByClass(SalaryHike.class);
	//name of mapper class
	job_obj.setMapperClass(SalaryMapper.class);
		// name of reducer class

	job_obj.setReducerClass(SalaryReducer.class);
